...
useSeoMeta({
	...
	description:
		"Khám phá các chương trình khuyến mãi hấp dẫn nhất từ AnhEm Motor - Trả góp 0%, ưu đãi tiền mặt và quà tặng cực khủng.",
});
...
const featuredPromotion = computed(() => {
	return (
		promotionStore.promotions.find((p) => p.isHot) ||
		promotionStore.promotions[0]
	);
});
...
const totalPromotionPages = computed(() =>
	Math.ceil(regularPromotions.value.length / itemsPerPage),
);
	...
	<div class="bg-white min-h-screen">
		...
		<main class="py-8 md:py-20 bg-gray-50/20 font-['Manrope']">
			<div class="container mx-auto px-4 md:px-8">
				<div
					v-if="promotionStore.promotions.length > 0"
					class="space-y-12 md:space-y-20"
				>
					<section
						v-if="
							activeTab === 'all' && promotionPage === 1 && featuredPromotion
						"
						class="transition-all duration-700"
					>
						<div
							class="inline-flex items-center gap-4 px-6 py-2.5 bg-red-600/5 rounded-full mb-12 border border-red-600/10 backdrop-blur-sm"
						>
							<Icon name="ph:fire-fill" class="text-red-600 animate-bounce" />
							<span
								class="text-[11px] font-black uppercase tracking-[0.2em] text-red-600"
								>Ưu đãi nổi bật nhất trong tuần</span
							>
						</div>
						...
						>
							<div
								class="lg:w-[60%] relative overflow-hidden aspect-video lg:min-h-[550px]"
							>
								<img
								...
								<div
									class="absolute bottom-6 md:bottom-12 left-6 md:left-12 p-4 md:p-8 bg-black/20 backdrop-blur-2xl border border-white/10 rounded-2xl md:rounded-[2.5rem]"
								>
									<div class="flex items-center gap-4 md:gap-6">
										<div class="text-center">
											<span
												class="block text-xl md:text-3xl font-black text-white leading-none"
												>03</span
											>
											<span
												class="text-[8px] md:text-[10px] font-black text-white/50 uppercase tracking-widest mt-1 md:mt-2 block"
												>Ngày</span
											>
										</div>
							...
							<div
								class="lg:w-[40%] p-8 md:p-16 lg:p-24 flex flex-col justify-center bg-white relative"
							>
								<div class="space-y-6 md:space-y-12">
									<div class="flex items-center gap-3 md:gap-5">
										<span
											class="px-4 md:px-6 py-1.5 md:py-2.5 bg-gray-950 text-white text-[8px] md:text-[10px] font-black uppercase tracking-widest rounded-lg md:rounded-2xl"
											>Hot Campaign</span
										>
										<span
											class="text-[8px] md:text-[10px] font-black text-red-600 uppercase tracking-[0.2em] flex items-center gap-2"
										>
									...
									<h2
										class="text-2xl md:text-4xl lg:text-6xl font-black text-gray-950 uppercase tracking-tight leading-[1.1] group-hover:text-red-600 transition-colors"
									>
									...
									<div
										class="pt-6 md:pt-12 border-t border-gray-100 flex items-center justify-between"
									>
										...
										<div
											class="flex items-center gap-2 md:gap-3 text-gray-950 font-black text-[10px] md:text-xs uppercase tracking-[0.1em] md:tracking-[0.2em] group-hover:translate-x-3 transition-transform duration-500"
										>
											Chi tiết
											<Icon
					...
					<section id="promotion-grid-section" class="space-y-16">
						<div
							class="flex items-end justify-between border-b border-gray-100 pb-10"
						>
							<div class="space-y-3">
								<h2
									id="promotion-grid-title"
									class="text-4xl font-black text-gray-950 uppercase tracking-tighter scroll-mt-[180px]"
								>
									Danh sách ưu đãi
								</h2>
								<p
								...
								>
									Khám phá và tận hưởng ngay hôm nay
								</p>
							</div>
							<div
								class="hidden sm:flex items-center gap-2 text-gray-400 text-[10px] font-black uppercase tracking-widest"
							>
								<span>Sắp xếp:</span>
								<button class="text-gray-950 flex items-center gap-2">
									Mới nhất <Icon name="ph:caret-down-fill" />
								</button>
							</div>
						</div>
				...
				>
					...
					<p class="text-gray-400 text-xl font-medium max-w-md mx-auto">
						Vui lòng quay lại sau để cập nhật các ưu đãi hấp dẫn nhất từ hệ
						thống AnhEm Motor.
					</p>
					<NuxtLink
						to="/"
		...
		>
			...
			<div class="container mx-auto px-4 relative z-10 text-center space-y-12">
				...
				>
					Hãy để đội ngũ chuyên gia của AnhEm Motor tư vấn chi tiết nhất về giá
					lăn bánh và các ưu đãi đặc quyền dành riêng cho bạn.
				</p>
				<div class="flex flex-wrap justify-center gap-6 pt-6">
					<NuxtLink
						to="/support"
						class="px-12 py-6 bg-red-600 text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-[0_25px_50px_-12px_rgba(230,0,35,0.6)] hover:scale-110 hover:-rotate-1 transition-all"
