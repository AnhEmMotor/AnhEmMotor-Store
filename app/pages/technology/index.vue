...
useSeoMeta({
  ...
  description:
    'Khám phá hệ sinh thái kết nối đa nền tảng, hiệu suất động cơ vượt trội và giải pháp quản lý kinh doanh thông minh tại AnhEm Motor.',
});
...
<template>
  <div class="tech-page bg-[#0a0a0a] text-white overflow-hidden relative">
    ...
    <section class="py-24 px-4 relative overflow-hidden text-center border-t border-white/5 z-10">
      ...
      <div
        class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-red-600/10 rounded-full blur-[120px] z-10 animate-pulse"
      />

      <div class="relative z-20 max-w-4xl mx-auto reveal reveal-up">
        <h2 class="text-4xl md:text-5xl font-black mb-8 uppercase tracking-tighter leading-tight">
          GIA NHẬP CỘNG ĐỒNG <br />
          <span class="text-red-600 underline decoration-white/20 underline-offset-8"
            >ANHEM MOTOR</span
          >
        </h2>
        <p class="text-lg text-gray-400 mb-12 max-w-2xl mx-auto leading-relaxed">
          Trải nghiệm dịch vụ di chuyển thông minh và các đặc quyền số độc duy nhất chỉ dành cho
          thành viên của chúng tôi.
        </p>
        <div class="flex flex-wrap justify-center gap-6">
          <NuxtLink
            to="/support"
            class="px-10 py-5 bg-red-600 text-white font-black rounded-xl hover:scale-110 hover:bg-red-700 transition-all flex items-center gap-4 shadow-[0_20px_50px_rgba(220,38,38,0.4)] group active:scale-95"
          >
            <Icon name="lucide:smartphone" class="text-xl group-hover:animate-bounce" />
            TẢI APP NGAY
          </NuxtLink>
          <NuxtLink
            to="/about"
            class="px-10 py-5 bg-white/5 backdrop-blur-md text-white font-black rounded-xl hover:bg-white/10 transition-all border border-white/10 flex items-center gap-4 hover:border-red-600/50 shadow-xl"
          >
            HỆ THỐNG SHOWROOM
            <Icon
              name="lucide:map-pinned"
              class="group-hover:translate-y-[-4px] transition-transform"
            />
          </NuxtLink>
        </div>
      </div>
