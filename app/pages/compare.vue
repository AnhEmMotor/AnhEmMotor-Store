  ...
  try {
    for (let i = 0; i < 3; i++) {
      if (
        ...
        !compareProducts.value.find((p) => p.id === detailedProducts.value[i].product.id)
      ) {
...
const specGroups = [
  {
    name: 'Động cơ',
    keys: ['engine_type', 'transmission_type'],
  },
  {
    name: 'Kích thước & Trọng lượng',
    keys: ['dimensions', 'weight', 'wheelbase', 'seat_height', 'ground_clearance', 'fuel_capacity'],
  },
  {
  ...
  {
    name: 'Công nghệ',
    keys: ['starter_system', 'fuel_system'],
  },
  {
    name: 'Tiện ích',
    keys: ['lighting_system', 'dashboard_type'],
  },
  {
...
const toggleSelect = (product) => {
  ...
  if (isSelected) {
    ...
    const index = detailedProducts.value.findIndex(
    ...
    <div class="max-w-[1400px] mx-auto px-6">
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div class="flex flex-col">
          ...
          >
            <Icon name="fa6-solid:chevron-left" class="mr-2 text-xs" />
            {{ isComparing ? 'Quay về danh sách xe' : 'Quay lại chọn loại xe' }}
          </button>

          <div v-if="!isComparing">
            <h1 class="text-3xl font-black text-gray-900 uppercase tracking-tighter leading-none">
              SO SÁNH THÔNG SỐ XE
              <span v-if="selectedVehicleType" class="text-[#CC0000]"
                >- {{ selectedVehicleType }}</span
              >
            </h1>
          </div>
          <h1
            v-else
            class="text-3xl font-black text-[#CC0000] uppercase tracking-tighter leading-none italic mt-4"
          >
            SO SÁNH SẢN PHẨM
          </h1>
        </div>
        ...
        <div class="flex items-center gap-4">
          <span v-if="!isComparing && selectedVehicleType" class="text-sm font-bold text-gray-400"
          ...
          <button
            ...
            :class="
              compareProducts.length >= 2
                ? 'bg-black text-white hover:bg-[#CC0000] cursor-pointer active:scale-95'
      ...
      >
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          >
            <div
              class="aspect-[4/3] w-full mb-6 relative z-10 flex items-center justify-center group-hover:scale-110 transition-transform duration-500"
            >
              <img
                :src="getRepresentativeImage(vt.id)"
                :alt="vt.label"
                class="max-w-full max-h-full object-contain mix-blend-multiply drop-shadow-xl"
                @error="$event.target.src = '/assets/image/placeholder-product.webp'"
              />
            </div>
            <h3
              class="text-sm lg:text-base font-black text-gray-900 group-hover:text-[#CC0000] uppercase tracking-widest relative z-10"
            >
              {{ vt.label }}
            </h3>
            <div
              class="absolute inset-0 bg-gradient-to-t from-red-50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"
            />
          </div>
        </div>
      </div>
      ...
      >
        <div
          ...
          class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 animate-pulse"
        >
        ...
        <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-8 gap-y-12">
          ...
          >
            <div>
              ...
              <div
                class="aspect-[4/3] mb-4 relative p-4 flex items-center justify-center transition-transform duration-300 group-hover:scale-105"
              >
                <img
                  :src="getBasicProductImage(product)"
                  :alt="product.name"
            ...
            <div class="flex items-center justify-between mt-4">
              <p class="text-xs font-medium text-gray-500">
                Giá từ:
                <span class="text-gray-900 font-black text-xs">{{
              ...
              <div
                ...
                :class="
                  compareProducts.some((p) => p.id === product.id)
                    ...
                    : 'bg-gray-100 text-transparent group-hover:border-gray-300'
                "
              >
          ...
          <div class="min-w-[900px] lg:min-w-0">
            <div class="bg-white pt-8 pb-4 border-b border-gray-200">
              <div class="grid grid-cols-12 items-stretch">
                ...
                >
                  ...
                  >
                    <div
                      class="aspect-[4/3] w-full mb-4 overflow-hidden flex items-center justify-center pt-4"
                    >
                      <img
                    ...
                    <div
                      class="w-full flex items-center justify-between bg-[#CC0000] text-white px-4 py-3"
                    >
                      ...
                      <div class="flex gap-4 shrink-0 ml-2">
                        ...
                        >
                          <Icon name="fa6-solid:arrows-rotate" class="text-sm" />
                        </button>
                        <button
                          class="hover:opacity-70 transition-opacity"
                          @click="removeProduct(item.product.id)"
                        >
                          <Icon name="fa6-solid:xmark" class="text-xl font-light" />
                        </button>
                      </div>
                    </div>
                    <div class="w-full bg-white border-x border-b border-gray-200 relative">
                      <select
                        v-model="item.currentVariant"
                        class="w-full text-[13px] font-black text-gray-700 py-4 px-4 outline-none cursor-pointer appearance-none bg-transparent relative z-10"
                      >
                        <option :value="item.currentVariant">
                          Phiên bản {{ item.currentVariant.name || 'Tiêu chuẩn' }}
                        </option>
                        <option v-for="v in item.otherVariants" :key="v.slug" :value="v">
                    ...
                    <div class="w-full text-center bg-white border-x border-b border-gray-200">
                      <NuxtLink
                        ...
                        class="block w-full py-4 text-[#CC0000] text-[11px] font-black uppercase tracking-widest hover:bg-red-50 transition-colors"
                      >
                  ...
                  >
                    <div class="mb-10 relative opacity-20 flex justify-center items-center">
                      ...
                      <Icon
                        name="fa6-solid:plus"
                        class="absolute top-0 right-0 text-3xl text-gray-500 font-bold"
                      />
                    </div>
                    <div class="w-full mt-auto">
                      <button
                        class="w-full py-4 border border-[#CC0000] text-[#CC0000] hover:bg-[#CC0000] hover:text-white text-[11px] font-black uppercase tracking-widest transition-colors flex justify-center items-center gap-2 bg-white group hover:bg-[#CC0000]"
                        @click="isModalOpen = true"
                      >
            ...
            <div class="relative">
              <div class="grid grid-cols-12 items-stretch border-b border-gray-200 bg-white">
                <template
                  v-for="(item, idx) in detailedProducts"
                  :key="'price-' + (item ? item.product.id : idx)"
                >
                  <div
                    v-if="item"
                    class="col-span-4 p-8 border-r border-gray-200 last:border-r-0 flex flex-col justify-center"
                  >
                    <h4 class="text-[17px] font-black text-[#CC0000] mb-3">Giá bán lẻ đề xuất</h4>
                    <span class="text-[15px] text-gray-900 font-medium">{{
                      formatPrice(item.currentVariant.price)
                    }}</span>
                  </div>
                  <div
                    v-else
                    class="col-span-4 p-8 border-r border-gray-200 last:border-r-0 flex flex-col justify-center bg-gray-50/50"
                  >
              ...
              <div
                ...
                :key="group.name"
                class="grid grid-cols-12 items-stretch border-b border-gray-200 bg-white"
              >
                <template
                  v-for="(item, idx) in detailedProducts"
                  :key="'group-' + gIdx + '-' + (item ? item.product.id : idx)"
                >
                  <div v-if="item" class="col-span-4 p-8 border-r border-gray-200 last:border-r-0">
                    ...
                    <div class="space-y-6">
                      <div v-for="key in group.keys" :key="'val-' + key">
                        <div class="text-[14px] text-gray-500 font-medium mb-1.5">
                          {{ specLabels[key] }}
                        </div>
                  ...
                  <div
                    v-else
                    class="col-span-4 p-8 border-r border-gray-200 last:border-r-0 bg-gray-50/50"
                  />
                </template>
              </div>
            </div>
        ...
        >
          ...
          <div class="pt-6 px-8 border-b border-gray-200 bg-white sticky top-0 z-0">
            <div class="flex gap-8 overflow-x-auto scrollbar-hide pr-12">
              <button
                class="pb-3 border-b-2 border-[#CC0000] text-black font-black text-[13px] tracking-widest whitespace-nowrap"
              >
                Tất cả
              </button>
              <button
                class="pb-3 border-b-2 border-transparent text-gray-600 font-medium hover:text-black text-[13px] tracking-widest whitespace-nowrap"
              >
                Xe tay ga
              </button>
              <button
          ...
          <div class="p-6 pb-2 bg-white sticky top-[57px] z-0">
            ...
            >
              <input
                ...
                class="w-full h-12 pl-4 pr-12 outline-none text-sm font-medium bg-transparent"
              />
              <Icon
                name="fa6-solid:magnifying-glass"
                class="absolute right-4 text-gray-500 text-lg"
              />
            </div>
          </div>
          ...
          <div class="flex-1 overflow-y-auto p-6 scrollbar-hide bg-white">
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-10">
              ...
              >
                <div>
                  <h3 class="text-[13px] font-black text-black leading-tight line-clamp-1 mb-2">
                    {{ product.name }}
                  </h3>
                  <div
                    class="aspect-[4/3] flex items-center justify-center p-2 transition-transform duration-300 group-hover:scale-105"
                  >
                    <img
                      :src="getBasicProductImage(product)"
                      :alt="product.name"
                ...
                <div>
                  <span
                    class="inline-block px-2 py-0.5 bg-red-600 text-white text-[9px] font-black uppercase mb-3"
                    >MỚI</span
                  >
                  <div class="flex items-center justify-between mt-1">
                    <p class="text-[11px] font-medium text-black">
                      Giá từ: <span class="font-black">{{ formatPrice(product.price) }}</span>
                    </p>
                    <div
                      ...
                      :class="
                        compareProducts.some((p) => p.id === product.id)
                          ...
                          : 'border-transparent bg-gray-100 text-transparent'
                      "
                    >
                      <Icon name="fa6-solid:check" class="text-[9px]" />
                    </div>
