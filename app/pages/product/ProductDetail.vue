...
const onAddToCart = () => {
	const cartItem = productMapper.toCartItem(
		detail.value,
	...
	<div class="min-h-screen bg-white">
		<div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-12 py-6">
			...
			<div v-if="error" class="text-center py-32 bg-gray-50 rounded-[3rem]">
				...
				<h2
					class="text-3xl font-black text-gray-900 mb-4 uppercase tracking-tighter"
				>
			...
			<div v-else class="space-y-16 lg:space-y-24">
				...
				<div class="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 relative">
					<!-- Spotlight Effect -->
					<div
						class="absolute -top-12 left-0 w-full h-full spotlight pointer-events-none"
					/>
					...
					<div class="lg:col-span-7 space-y-8">
						...
						<div class="grid grid-cols-1 md:grid-cols-12 gap-8 pt-4">
							...
							>
								...
								<p
									class="text-gray-500 font-medium leading-relaxed max-w-xl text-base"
								>
									{{
						...
						<div class="sticky top-28 space-y-8">
							<div class="space-y-4">
								<div class="flex items-center gap-3">
									...
									<span
										class="text-[9px] font-black text-primary uppercase tracking-[0.4em]"
										>{{ detail.product.category }}</span
									>
								</div>
								<h1
							...
							<div
								v-if="Object.keys(variantGroups).length > 0"
								class="space-y-4"
							>
								<div class="space-y-3">
									<label
									...
									<div class="flex flex-wrap items-center gap-4">
										<div class="relative flex-grow">
											...
											>
												<option
													v-for="(variants, name) in variantGroups"
													:key="name"
													:value="name"
												>
													{{ name }}
									...
									<template v-if="isMotorbike">
										...
										>
											<div class="flex items-center gap-2">
												TƯ VẤN & BÁO GIÁ
												<Icon
													name="fa6-solid:phone-volume"
													class="text-[10px] animate-bounce"
												/>
											</div>
											<span
				...
				<div
					...
					class="space-y-24 lg:space-y-32 relative"
				>
					<div
					...
					<div class="text-center space-y-4 reveal-up">
						<div
							class="inline-flex items-center gap-2 px-4 py-1.5 bg-primary/5 rounded-full"
						>
							<div class="w-1.5 h-1.5 bg-primary rounded-full animate-pulse" />
							<span
								class="text-[8px] font-black text-primary uppercase tracking-[0.3em]"
								>Innovation</span
							>
						</div>
						<h2
				...
				<div class="space-y-20 relative">
					...
					<div class="text-center space-y-6 reveal-up">
						...
						<p
							class="text-gray-400 font-bold text-xs uppercase tracking-widest max-w-lg mx-auto"
						>
							Chúng tôi không chỉ bán xe, chúng tôi trao gửi sự an tâm tuyệt đối
							và đặc quyền tối thượng
						</p>
					</div>
					...
					<div class="max-w-6xl mx-auto relative px-4">
						...
						>
							...
							>
								...
								/>
								<div
									class="absolute -bottom-10 -left-10 w-40 h-40 bg-primary/5 blur-[80px]"
								/>
							...
							<!-- Right: Competitors -->
							<div
								class="lg:col-span-4 p-8 lg:p-12 space-y-8 bg-white/30 text-center"
							>
					...
					>
						<div class="space-y-8 reveal-up">
							...
							>
								...
								/>
								<span
									class="text-[9px] font-black text-primary uppercase tracking-[0.3em]"
							...
							<p
								class="text-white/60 font-bold text-lg max-w-md leading-relaxed"
							>
								Sở hữu ngay siêu phẩm {{ detail.product.name }} với ưu đãi độc
								quyền chỉ có tại hệ thống AnhEm Motor.
							</p>
						...
						<div
							class="relative lg:h-[400px] flex items-center justify-center reveal-up delay-200"
						>
							<div
				...
				<div v-if="specifications.length > 0" class="space-y-16">
					...
					>
						<div class="space-y-3">
							<span
								class="text-[9px] font-black text-primary uppercase tracking-[0.6em]"
						...
						>
							...
							>
								<div class="overflow-hidden">
									<div
										class="grid grid-cols-1 gap-6 pt-6 border-t border-gray-100/50"
									>
										<div
										...
										>
											...
											<span
												class="text-base font-black text-gray-900 tracking-tight pl-3.5"
												>{{ spec.value }}</span
											>
										</div>
									</div>
