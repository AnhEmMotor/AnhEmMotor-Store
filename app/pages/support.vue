...
const handleContactSubmit = async (data) => {
  ...
  const result = await contactStore.submitContact(formData);
  if (result?.success && resetForm) {
    resetForm();
  }
};
onMounted(() => {
  contactStore.restoreTracking();
...
<template>
  <div class="bg-white min-h-screen overflow-x-hidden">
    ...
    <section
      class="relative py-24 bg-white overflow-hidden border-t border-gray-100 reveal reveal-up"
    >
      <div
        class="absolute top-0 right-0 w-1/3 h-full bg-red-600/5 -skew-x-12 transform translate-x-1/4"
      />
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-20 items-stretch">
          <div class="flex flex-col reveal reveal-left">
            ...
            <div class="mt-10 space-y-6">
              <div
                class="flex items-start gap-6 p-6 rounded-[2.5rem] border border-gray-100 hover:shadow-xl hover:shadow-gray-100 transition-all bg-gray-50/30 group"
              >
                <div
                  class="w-14 h-14 rounded-2xl bg-white shadow-sm flex items-center justify-center shrink-0 group-hover:bg-primary transition-colors duration-500"
                >
                  <Icon
          ...
          >
            ...
            <SupportRequestTracker
              v-if="contactStore.trackingAccess"
              :request="contactStore.trackingRequest"
              :loading="contactStore.trackingLoading"
              :rating-submitting="contactStore.ratingSubmitting"
              @refresh="contactStore.fetchTracking"
