<template>
	<div class="min-h-screen bg-white test-drive-page">
		<section
			class="relative h-[45vh] min-h-[400px] flex items-center overflow-hidden"
		>
			...
			<div class="container mx-auto px-6 relative z-10">
				<div class="max-w-2xl space-y-4">
					<div
						class="inline-flex items-center gap-2 px-3 py-1 bg-primary/20 backdrop-blur-xl border border-primary/30 rounded-full"
					>
						<span class="w-1 h-1 bg-primary rounded-full" />
						<span
							class="text-[10px] font-extrabold text-white uppercase tracking-[0.3em]"
							>Hành trình trải nghiệm</span
						>
					</div>
					<h1
					...
					<p
						class="text-[16px] md:text-[18px] text-gray-300 font-medium max-w-md leading-relaxed border-l-4 border-primary pl-4"
					>
				...
				<div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
					<div class="lg:col-span-4 space-y-6 order-1">
						...
						>
							...
							>
								...
								<div class="space-y-2 pt-2">
									...
									<div class="grid grid-cols-2 gap-y-3 gap-x-4">
										<div
											v-for="spec in selectedVehicle.specs"
											:key="spec.l"
										...
										>
											<span
												...
												>{{ spec.l }}</span
											>
											<span class="text-[12px] font-extrabold text-gray-900">{{
												spec.v
											}}</span>
										</div>
									</div>
							...
							<div
								v-else
								class="text-center space-y-4 py-8 group-hover:scale-105 transition-transform"
							>
								<div
									class="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-2 group-hover:bg-primary/5 transition-colors"
								>
									<Icon
										name="ph:motorcycle-bold"
						...
						<div class="grid grid-cols-2 gap-4">
							<div
								class="bg-gray-950 rounded-[20px] p-4 text-white text-center"
							>
								<p class="text-[22px] font-extrabold leading-none">500+</p>
								<p
									class="text-[10px] font-semibold text-primary uppercase tracking-widest mt-1"
								>
									Lượt trải nghiệm
								</p>
							</div>
							<div
								class="bg-gray-50 rounded-[20px] p-4 border border-gray-100 text-center"
							>
								<p
									class="text-[22px] font-extrabold text-gray-900 leading-none"
								>
									98%
								</p>
								<p
									class="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mt-1"
								>
									Độ hài lòng
								</p>
							</div>
					...
					<div class="lg:col-span-8 order-2">
						<div
							class="bg-white rounded-[20px] shadow-xl border border-gray-100 overflow-hidden"
						>
							<div class="p-6 lg:p-8 space-y-8">
								...
								>
									<div class="space-y-4">
										<div class="flex items-center gap-2">
											<span
												class="text-[12px] font-extrabold text-primary uppercase tracking-widest"
												>01. Thông tin liên hệ</span
											>
										...
										<div class="space-y-4">
											<div class="relative">
												...
												<Icon
													...
													class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 text-[10px]"
												/>
											</div>
											<div class="grid grid-cols-3 gap-3">
												...
												<select
													v-model="form.hour"
													class="premium-input text-[12px] font-extrabold"
												>
													<option
									...
									<div class="pt-2 space-y-4">
										<label
											class="flex items-start gap-2 p-3 bg-gray-50 rounded-lg cursor-pointer"
										>
											<input
										...
										<button
											...
											class="w-full py-4 bg-gradient-premium text-white font-extrabold text-[12px] uppercase tracking-[0.2em] rounded-lg shadow-glow transition-all"
										>
										...
										<div class="grid grid-cols-3 gap-2">
											...
											>
												<Icon
													:name="trust.i"
													class="text-primary text-sm mb-1"
												/>
												<span
													class="text-[9px] font-extrabold text-gray-900 uppercase"
													>{{ trust.t }}</span
												>
											</div>
										</div>
								...
								<div v-else class="text-center py-8 space-y-4">
									...
									<NuxtLink
										to="/products"
										class="inline-block px-6 py-3 bg-gray-900 text-white font-extrabold text-[11px] uppercase rounded-lg"
										>Quay lại danh sách xe</NuxtLink
									>
								</div>
							</div>
						</div>
					...
					<div class="space-y-6">
						...
						<div class="grid grid-cols-1 gap-3">
							<div
								...
								:key="idx"
								class="flex gap-3 items-center p-3 bg-white rounded-xl shadow-sm"
							>
								<div
									class="w-8 h-8 rounded-lg bg-primary/5 flex items-center justify-center text-primary font-bold text-[16px] shrink-0"
								>
									{{ idx + 1 }}
								</div>
								<div class="space-y-1.5">
									<h5
										class="text-[15px] font-extrabold uppercase leading-tight tracking-tight"
									>
										{{ step.t }}
									</h5>
									<p class="text-[14px] text-gray-500 leading-normal">
										{{ step.d }}
									</p>
								</div>
							</div>
...
const processSteps = [
	{
		...
		d: "Lựa chọn dòng xe yêu thích, ngày và khung giờ phù hợp ngay trên website.",
	},
	{
		t: "Xác nhận lịch hẹn",
		d: "Tư vấn viên sẽ liên hệ trong vòng 30 phút để xác nhận và chuẩn bị sẵn xe.",
	},
	{
		t: "Check-in & Thủ tục",
		d: "Đến showroom, cung cấp CCCD và Bằng lái để hoàn tất hồ sơ bảo hiểm nhanh.",
	},
	{
		t: "Hướng dẫn kỹ thuật",
		d: "Chuyên viên giới thiệu chi tiết tính năng, công nghệ và lưu ý an toàn trước khi cầm lái.",
	},
	{
		t: "Trải nghiệm thực tế",
		d: "Cầm lái trải nghiệm sức mạnh động cơ và sự linh hoạt trên cung đường thiết kế riêng.",
	},
];
		...
		if (res?.items && res.items.length > 0) {
			...
			variants.value = vehicleProducts.flatMap((product) =>
				(product.variants || []).map((v) => ({
					...
					engine: product.specifications?.find((s) => s.key === "Displacement")
						?.value,
					brake: product.specifications?.find((s) => s.key === "FrontBrake")
						?.value,
					fuel: product.specifications?.find((s) => s.key === "FuelConsumption")
						?.value,
				})),
			);
		} else {
...
.shadow-glow {
	box-shadow: 0 8px 20px -5px rgba(227, 24, 55, 0.4);
}
