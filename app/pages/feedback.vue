    ...
    <div class="container mx-auto px-6 relative z-10 pt-24 pb-12 flex-grow">
      <div class="max-w-3xl mx-auto">
        <div class="text-center mb-6 fb-reveal">
          ...
          <h1 class="fb-main-title">Đóng góp <span class="fb-text-red">ý kiến</span></h1>
          <p class="fb-subtitle">
            Bạn muốn AnhEm Motor cải thiện điều gì? Mọi ý kiến của bạn là chìa khóa để chúng tôi
            nâng tầm dịch vụ mỗi ngày.
          </p>

          <div class="fb-trust-row">
            <div
              v-for="trust in [
                'Mọi góp ý đều được xem xét',
...
.fb-bg-overlay {
  ...
  background:
    radial-gradient(
      ...
      rgba(253, 253, 253, 0.82) 28%,
      rgba(253, 253, 253, 0.35) 66%,
      rgba(253, 253, 253, 0.18) 100%
    ),
    linear-gradient(
      to bottom,
      rgba(253, 253, 253, 0.46) 0%,
      rgba(253, 253, 253, 0.18) 46%,
      rgba(253, 253, 253, 0.86) 100%
    );
}
.fb-glow {
  position: absolute;
...
.fb-card-highlight {
  ...
  width: 100%;
  height: 1px;
  background: linear-gradient(to right, transparent, rgba(227, 24, 55, 0.2), transparent);
}
.fb-form-inner {
  padding: 1.75rem;
  display: flex;
...
.fb-emoji-btn:hover .fb-emoji-box {
  background: #fff2f2;
  color: #e31837;
  transform: scale(1.08);
  box-shadow: 0 6px 16px rgba(227, 24, 55, 0.12);
  border: 1px solid #fecaca;
}
.fb-emoji-btn.active .fb-emoji-box {
  background: #e31837;
  color: #fff;
  transform: scale(1.1);
  box-shadow: 0 8px 20px rgba(227, 24, 55, 0.25);
}
.fb-emoji-label {
  font-size: 10px;
  font-weight: 600;
...
.fb-emoji-btn.active .fb-emoji-label {
  color: #e31837;
  font-weight: 800;
}
...
.fb-input-box:focus-within .fb-input-icon {
  color: #e31837;
}
.fb-input:focus {
  border-color: #e31837;
...
.fb-submit-btn {
  width: 100%;
  padding: 1.25rem;
  background: linear-gradient(135deg, #ff2d55 0%, #e31837 100%);
  color: #fff;
  font-weight: 800;
  font-size: 14px;
  text-transform: uppercase;
...
.fb-submit-btn:hover {
  ...
  box-shadow: 0 12px 32px rgba(227, 24, 55, 0.28);
}
...
.fb-reveal {
  animation: fbReveal 0.8s cubic-bezier(0.16, 1, 0.3, 1) forwards;
}
@keyframes fbReveal {
  from {
    opacity: 0;
