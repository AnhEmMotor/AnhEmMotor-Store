				...
				<div class="lg:w-2/3 xl:w-3/4">
					...
					>
						<div class="border-b border-gray-100 pb-5 mb-6">
							...
							<p class="text-sm text-gray-500 mt-1">
								Đảm bảo tài khoản của bạn đang sử dụng một mật khẩu mạnh và an
								toàn.
							</p>
						</div>
							...
							<div class="space-y-6 max-w-lg">
								...
								<div>
									...
									<p class="mt-2 text-xs text-gray-500">
										Tối thiểu 6 ký tự, bao gồm ít nhất 1 chữ số và 1 ký tự đặc
										biệt (VD: @, #, $, ...).
									</p>
								</div>
...
watch(
	...
	(newUser) => {
		if (newUser) {
			...
			formData.value.gender = newUser.gender || "";
		}
	},
	{ immediate: true },
);
...
async function handleUpdatePassword() {
	...
	if (!/[^a-zA-Z0-9]/.test(data.newPassword)) {
		return instance.$toast.error(
			"Mật khẩu mới phải chứa ít nhất 1 ký tự đặc biệt (không phải chữ hoặc số)!",
		);
	}
