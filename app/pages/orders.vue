...
useSeoMeta({
  ...
  description: 'Xem danh sách và trạng thái các đơn hàng bạn đã mua tại AnhEm Motor.',
});
...
const handleUpdateOrder = async () => {
  ...
  try {
    ...
    const message =
      error.response?.data?.Errors?.[0]?.Message || 'Có lỗi xảy ra khi cập nhật đơn hàng.';
    toast.error(message);
  } finally {
...
const confirmCancel = async () => {
  ...
  try {
    ...
    const message = error.response?.data?.Errors?.[0]?.Message || 'Có lỗi xảy ra khi hủy đơn hàng.';
    toast.error(message);
  } finally {
  ...
  <div class="bg-gray-50 min-h-screen py-12">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="mb-8">
        ...
        <p class="text-gray-500">Quản lý và theo dõi trạng thái các đơn hàng của bạn.</p>
      </div>
