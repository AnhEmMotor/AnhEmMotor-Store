...
const productMapper = {
  ...
  mapProductItem(raw) {
    const variants = (raw.variants || []).map((v) => ({
      ...
      colors: (v.colors || []).map((c) => ({
        ...
        effectiveMax:
          c.maxPurchaseQuantity ?? v.effectiveMax ?? v.productLimit ?? v.product_limit ?? null,
      })),
    ...
    return {
      ...
      brandId:
        raw.brandId ??
        raw.brand_id ??
        (typeof raw.brand === 'object' ? raw.brand?.id : null) ??
        null,
      brand: raw.brandName || (typeof raw.brand === 'string' ? raw.brand : raw.brand?.name) || '',
      type: raw.type || '',
      rating: raw.rating || 5,
  ...
  mapProductDetail(raw, attributeLabels = null) {
    ...
    if (currentVariant.cover_image_url) {
      if (
        ...
        const urls = currentVariant.cover_image_url.split(',').map((u) => u.trim());
    ...
    const mapColors = (variantLike) => {
      if (Array.isArray(variantLike.colors) && variantLike.colors.length > 0) {
        return variantLike.colors.map((c) => ({
          ...
          code: c.colorCode || c.color_code || c.code || '#ccc',
          image: getImageUrl(
            c.coverImageUrl || c.cover_image_url || c.image || variantLike.cover_image_url
          ),
          effectiveMax:
            c.maxPurchaseQuantity ?? variantLike.effectiveMax ?? variantLike.product_limit ?? null,
          maxPurchaseQuantity: c.maxPurchaseQuantity,
        }));
      }
  ...
  toSeoMeta(detail) {
    ...
    return {
      ...
      twitterImage: variant?.image || '/assets/image/index/index-banner-bg.webp',
    };
  },
