			...
			<div class="checkout-modal-content" role="dialog" aria-modal="true">
				...
				<div class="checkout-body">
					<section class="payment-methods" aria-label="Phương thức thanh toán">
						...
						>
							<div class="pm-icon" :style="{ background: method.color }">
								{{ method.icon }}
							</div>
							<div class="pm-meta">
					...
					<section class="order-summary bg-gray-50 p-4 rounded-lg mt-4">
						...
						<div
							...
							class="mt-3 space-y-3 max-h-48 overflow-auto"
						>
							<div
								v-for="item in cart"
								:key="item.id"
						...
						<div class="mt-4 space-y-2">
							...
							<div v-if="selectedMethod === 'cod'" class="flex justify-between">
								<span>Phí COD:</span><span>{{ formatCurrency(codFee) }}</span>
							</div>
				...
				<footer class="checkout-footer flex items-center justify-end gap-3">
					...
					<button class="btn-confirm primary" @click="$emit('confirm')">
						Xác nhận thanh toán
					</button>
				</footer>
			</div>
...
const methods = [
	{
		...
		description: "Chuyển khoản qua ATM, Internet Banking hoặc Mobile Banking",
		note: "Thời gian xử lý: Ngay lập tức",
		color: "#dc2626",
		icon: "🏦",
	},
	{
