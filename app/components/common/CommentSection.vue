...
<template>
	<div class="mt-12 border-t border-gray-100 pt-8 w-full">
		<h3 class="text-xl font-black uppercase tracking-tight text-gray-950 mb-8 flex items-center gap-2">
		...
		<div class="flex gap-4 mb-10">
			...
			<div class="flex-grow">
				<div class="relative rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden focus-within:border-red-500 focus-within:ring-1 focus-within:ring-red-500 transition-all">
					<textarea
						v-model="newComment"
						rows="3"
		...
		<div v-else-if="comments.length === 0" class="text-center py-10 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
			...
			<p class="text-gray-500 font-medium">Chưa có bình luận nào. Hãy là người đầu tiên bình luận!</p>
		</div>
        <div v-else class="space-y-6">
            <div v-for="comment in comments" :key="comment.id" class="flex gap-4">
                ...
                <div class="flex-grow bg-gray-50 p-4 rounded-2xl rounded-tl-none border border-gray-100">
                    ...
                    <p class="text-gray-700 whitespace-pre-wrap text-sm leading-relaxed">{{ comment.content }}</p>
                </div>
            </div>
        </div>
	</div>
