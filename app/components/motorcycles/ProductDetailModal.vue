<template>
	...
	>
		<div
			v-if="product"
			class="bg-white rounded-lg shadow-lg max-w-sm sm:max-w-lg md:max-w-3xl w-full overflow-auto"
		>
			<div class="flex justify-end p-2 sm:p-3">
				<button
					class="text-xl sm:text-2xl text-gray-600 hover:text-gray-900"
					@click="emit('close')"
				>
					&times;
				</button>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 sm:p-6">
				<div class="flex items-center justify-center">
					<img
						:src="product.image"
						:alt="product.name"
						class="max-h-60 sm:max-h-80 object-contain"
					>
				</div>
				<div>
					...
					<h4 class="text-lg sm:text-xl text-red-600 font-bold mb-4">
