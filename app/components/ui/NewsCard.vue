	...
	>
		...
		<div class="p-6 md:p-8 flex flex-col flex-1">
			<div class="flex items-center gap-3 mb-4">
				<div
					class="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center group-hover:bg-red-50 transition-colors"
				>
					<Icon
						name="ph:calendar-blank-fill"
						class="text-xs text-gray-400 group-hover:text-red-500 transition-colors"
					/>
				</div>
				<span
				...
				>
					{{
						news.date || new Date(news.createdAt).toLocaleDateString("vi-VN")
					}}
				</span>
			</div>

			<h3
				class="text-base md:text-xl font-black text-gray-900 uppercase tracking-tight leading-tight mb-3 group-hover:text-red-600 transition-colors line-clamp-2"
			>
				{{ news.title }}
			</h3>
			...
			>
				<div class="flex flex-col">
					<span
						class="text-[8px] md:text-[9px] font-black text-gray-400 uppercase tracking-widest mb-0.5"
						>Chi tiết</span
					>
					<span
						class="text-sm font-black text-gray-900 uppercase tracking-tighter group-hover:text-red-600 transition-colors"
						>Đọc bài viết</span
					>
				</div>
				<div
