  ...
  <section class="py-12 md:py-20 bg-white relative overflow-hidden font-['Manrope']">
    ...
    <div class="container mx-auto px-4 relative z-10">
      ...
      <div class="grid grid-cols-4 gap-2 md:gap-4 relative">
        <div
          class="absolute top-[35px] md:top-[60px] left-[12%] right-[12%] h-[1.5px] bg-gray-100 z-0"
        >
          <div class="absolute inset-0 bg-red-600/30 scale-x-100 origin-left" />
        </div>

        <div v-for="(step, index) in steps" :key="index" class="relative group z-10">
          <div class="flex flex-col items-center text-center space-y-4 md:space-y-8">
            <div class="relative">
              <div
                class="w-14 h-14 md:w-32 md:h-32 rounded-xl md:rounded-[2.5rem] bg-white flex items-center justify-center border border-gray-100 group-hover:scale-110 transition-all duration-700 shadow-lg group-hover:shadow-red-600/10"
              >
                <img
                  :src="step.icon"
              ...
              <div
                class="absolute -top-2 -right-2 md:-top-4 md:-right-4 w-6 h-6 md:w-12 md:h-12 rounded-full bg-gray-950 text-white flex items-center justify-center font-black text-[8px] md:text-xs shadow-xl border-2 md:border-4 border-white"
              >
                {{ index + 1 }}
              </div>
            </div>
            ...
            <div class="space-y-1 md:space-y-4">
              <h3
                class="text-[9px] md:text-xl font-[1000] text-gray-950 uppercase tracking-tight group-hover:text-red-600 transition-colors leading-tight"
              >
                {{ step.title }}
              </h3>
              <p class="hidden md:block text-gray-500 text-sm font-medium leading-relaxed">
                {{ step.desc }}
              </p>
            </div>
          </div>
