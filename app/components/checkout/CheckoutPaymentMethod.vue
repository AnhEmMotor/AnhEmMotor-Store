...
const subtotal = computed(() => {
  return (cartDetails.value || []).reduce((sum, item) => sum + item.price * item.quantity, 0);
});

const requiresDeposit = computed(() => {
  const orderValueExceeds = Number(orderStore.settings.Order_value_exceeds || 0);
...
onMounted(() => {
  if (
    ...
    orderStore.shippingInfo.paymentMethod = requiresDeposit.value ? 'PayOS' : 'COD';
