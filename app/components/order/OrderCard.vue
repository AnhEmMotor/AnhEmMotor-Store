        ...
        <span
          ...
          :class="{
            'bg-yellow-50 text-yellow-600':
              order.statusId === 'pending' || order.statusId === 'waiting_deposit',
            'bg-blue-50 text-blue-600':
              order.statusId === 'confirmed_cod' ||
              order.statusId === 'deposit_paid' ||
              order.statusId === 'paid_processing',
            'bg-indigo-50 text-indigo-600':
              order.statusId === 'delivering' || order.statusId === 'waiting_pickup',
      ...
      >
        ...
        <div class="flex-1">
          ...
          <p class="text-sm font-bold text-blue-700 leading-tight">
            Vui lòng đặt cọc
            <span class="text-red-500">{{ formatPrice(order.depositAmount) }}</span>
            ({{ order.depositRatio || 50 }}%) để xác nhận đơn. Liên hệ hotline hoặc chờ Admin gọi
            xác nhận.
          </p>
        </div>
      </div>
        ...
        <div class="space-y-3">
          ...
          <div class="bg-gray-50/50 rounded-2xl p-4 space-y-2">
            ...
            <div class="flex items-start gap-3">
              <Icon name="fa6-solid:location-dot" class="text-[10px] text-gray-400 mt-1" />
              <span class="text-sm font-medium text-gray-600 leading-relaxed">{{
                [order.customer?.address, order.customer?.wardName, order.customer?.provinceName]
                  .filter(Boolean)
    ...
    >
      ...
      <div class="text-right space-y-1">
        <div v-if="timeRemaining" class="mb-3 flex flex-col items-end">
          <p
            class="text-[10px] text-gray-400 font-black uppercase tracking-widest flex items-center gap-1"
          >
        ...
        <div
          v-if="
            (order.statusId === 'deposit_paid' || order.statusId === 'delivering') &&
