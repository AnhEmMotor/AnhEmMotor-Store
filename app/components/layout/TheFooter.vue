  ...
  >
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-10">
        ...
        <div v-for="section in footerData.sections" :key="section.title" class="footer-col">
          <h3 class="footer-heading">{{ section.title }}</h3>
          <ul class="space-y-2">
            <li v-for="link in section.links" :key="link.name">
              <RouterLink :to="link.url" class="footer-link">{{ link.name }}</RouterLink>
            </li>
          </ul>
        </div>
      </div>
      ...
      <div class="text-center pt-8 border-t border-white/10 text-white/60">
        <p>© 2026 AnhEm Motor. Mọi quyền được bảo lưu. | Thiết kế bởi AnhEm Team</p>
      </div>
    </div>
  </footer>
</template>
