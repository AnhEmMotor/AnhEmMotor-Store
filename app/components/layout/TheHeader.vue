				...
				<nav class="hidden xl:block">
					<ul class="flex items-center gap-4">
						...
						<li class="group relative py-2">
							<NuxtLink
								to="/products?category_ids=8"
								class="nav-link-refined"
								:class="{ active: isRouteActive('/products?category_ids=8') }"
							>
								XE MÁY
								<span
							...
							<div
								class="absolute top-full -left-5 w-max max-w-225 bg-white shadow-[0_30px_60px_-15px_rgba(0,0,0,0.2)] border border-gray-50 rounded-4xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-500 p-5 z-100 translate-y-4 group-hover:translate-y-0"
							>
								<div class="flex items-center gap-2">
									<NuxtLink
										v-for="cat in dynamicMotorcycleCategories"
										:key="cat.name"
										:to="cat.path"
										class="flex flex-col items-center gap-3 p-5 rounded-[1.5rem] hover:bg-primary/5 group/cat transition-all text-center min-w-35"
									>
										<div
											class="w-14 h-14 bg-gray-50 rounded-2xl flex items-center justify-center group-hover/cat:bg-white transition-all duration-500 shadow-sm group-hover/cat:shadow-md group-hover/cat:scale-110 group-hover/cat:-rotate-3"
										>
											<Icon
												:name="getCategoryIcon(cat)"
												class="text-2xl text-gray-400 group-hover/cat:text-primary"
											/>
										</div>
										<div class="flex flex-col">
											<span
												class="text-[10px] sm:text-[11px] font-black text-gray-900 uppercase tracking-wider group-hover/cat:text-primary transition-colors whitespace-nowrap"
												>{{ cat.name }}</span
											>
											<span
												class="text-[9px] text-gray-400 font-bold uppercase tracking-tight"
												>{{ getCategoryDesc(cat) }}</span
											>
										</div>
									</NuxtLink>
								</div>
						...
						<li>
							<NuxtLink
								to="/products?category_ids=12,13"
								class="nav-link-refined"
								:class="{
									active: isRouteActive('/products?category_ids=12,13'),
								}"
								>PHỤ TÙNG & PHỤ KIỆN<span
									class="nav-dot-floating"
									:class="{
										active: isRouteActive('/products?category_ids=12,13'),
									}"
...
const dynamicMotorcycleCategories = computed(() => {
	const allCats = Array.isArray(categoryStore.categories)
