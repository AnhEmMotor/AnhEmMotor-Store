...
v-for="(job, index) in jobs" :key="job.id" 
				...
				>
					...
					<div class="flex gap-4">
						<button class="flex-1 px-6 py-4 bg-gray-50 text-gray-900 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-gray-100 transition-all border border-transparent hover:border-gray-200" @click="selectedJob = job">
							Chi tiết
						</button>
						<button class="flex-1 px-6 py-4 bg-primary text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:scale-105 transition-all shadow-xl shadow-primary/20 active:scale-95" @click="applyForJob(job)">
							Ứng tuyển
						</button>
					</div>
				</div>
					...
					>
						...
						<div v-if="selectedJob" class="px-10 pb-10 relative mt-[-60px]">
							<div class="flex justify-between items-start mb-10">
								...
								<button class="mt-12 p-3 bg-gray-50 hover:bg-primary hover:text-white rounded-2xl transition-all text-gray-400" @click="selectedJob = null">
								...
								<div v-for="(section, sIndex) in selectedJob.details" :key="sIndex" class="space-y-6">
									...
									<ul class="grid grid-cols-1 gap-4 pl-14">
										<li v-for="(point, pIndex) in section.points" :key="pIndex" class="flex items-start gap-4 text-gray-500 font-medium leading-relaxed group/item">
											<div class="mt-1.5 w-2 h-2 rounded-full bg-primary/20 group-hover/item:bg-primary transition-colors flex-shrink-0"/>
											{{ point }}
										</li>
									</ul>
								</div>
							...
							<div class="mt-12 pt-8 border-t border-gray-100 flex gap-6">
								<button class="flex-1 py-5 bg-gray-50 text-gray-900 rounded-[1.5rem] font-black text-[11px] uppercase tracking-[0.2em] hover:bg-gray-100 transition-all" @click="selectedJob = null">
									Quay lại
								</button>
								<button class="flex-1 py-5 bg-primary text-white rounded-[1.5rem] font-black text-[11px] uppercase tracking-[0.2em] hover:scale-[1.02] active:scale-95 shadow-[0_20px_40px_rgba(227,24,55,0.25)] transition-all flex items-center justify-center gap-3" @click="applyFromModal">
									Nộp hồ sơ ngay
