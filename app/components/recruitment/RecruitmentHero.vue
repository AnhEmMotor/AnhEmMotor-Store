<template>
	<section
		class="relative min-h-screen flex items-center overflow-hidden bg-[#0a0a0a] pt-20"
	>
		<div class="absolute inset-0 z-0">
			...
			/>
			<div
				class="absolute inset-0 bg-gradient-to-t from-[#0a0a0a]/80 via-transparent to-transparent"
			/>
		</div>
		...
		/>
		<div
			class="absolute bottom-1/4 left-1/4 w-64 h-64 bg-primary/10 rounded-full blur-[100px] animate-pulse delay-700"
		/>

		<div
			class="absolute top-1/3 left-1/4 w-12 h-12 border border-white/10 rounded-lg rotate-12 animate-float-slow opacity-20"
		/>
		<div
			class="absolute bottom-1/3 right-1/3 w-16 h-16 border border-primary/20 rounded-full animate-float-slow delay-1000 opacity-30"
		/>
		...
		<div class="container mx-auto px-6 relative z-10">
			<div class="max-w-4xl">
				...
				<p
					class="text-lg text-white/60 mb-10 leading-relaxed max-w-xl animate-fade-in-up delay-200 font-medium"
				>
					Chúng tôi không chỉ bán xe, chúng tôi xây dựng những hành trình đầy
					cảm hứng. Hãy cùng bứt phá trong môi trường chuyên nghiệp và đầy cơ
					hội.
				</p>
				...
				<div
					class="grid grid-cols-2 md:grid-cols-3 gap-10 mt-16 pt-10 border-t border-white/5 animate-fade-in-up delay-400"
				>
					<div
						v-for="stat in [
							{ v: '10+', l: 'Năm kinh nghiệm' },
							{ v: '50+', l: 'Cộng sự tài năng' },
							{ v: '98%', l: 'Mức độ hài lòng' },
						]"
						:key="stat.l"
					>
						<div
						...
						<div
							class="text-white/40 text-[9px] font-black uppercase tracking-widest"
						>
							{{ stat.l }}
						</div>
					</div>
		...
		<div
			class="absolute bottom-0 left-0 w-full h-32 bg-gradient-to-t from-white to-transparent z-20"
		/>
	</section>
</template>

<script setup>
const emit = defineEmits(["scroll-to-jobs", "scroll-to-form"]);

const scrollToJobs = () => emit("scroll-to-jobs");
const scrollToForm = () => emit("scroll-to-form");
</script>
