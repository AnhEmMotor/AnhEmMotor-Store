...
const modeText = computed(() => (props.mode === 'register' ? 'Đăng ký' : 'Đăng nhập'));
  ...
  window.FB.login(
    (response) => {
      if (response.authResponse) {
        (async () => {
          try {
            const { error } = await authStore.loginWithFacebook(response.authResponse.accessToken);
            if (!error) {
              await navigateTo(route.query.redirect || '/');
            } else {
  ...
  if (window.google) {
    ...
    const timer = setInterval(() => {
      ...
      if (++checkCount > 10) {
        clearInterval(timer);
        isLoading.value = false;
        instance.$toast.error('Google SDK chưa sẵn sàng. Vui lòng thử lại sau vài giây!');
      }
    }, 500);
  }
};
...
<template>
  <div class="space-y-4">
    <div class="relative flex items-center justify-center py-2">
      ...
      <span class="bg-white px-2 text-xs font-medium uppercase tracking-wider text-gray-400 mt-3"
