  ...
  >
    ...
    >
      ...
      >
        ...
        <div
          class="absolute top-8 left-8 w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-2xl border border-gray-50 transform group-hover:-translate-y-2 group-hover:rotate-6 transition-all duration-500"
        >
          <Icon
            v-if="!isEmoji(service.icon)"
            :name="service.icon || 'fa6-solid:wrench'"
    ...
    >
      <div class="space-y-4">
        <div class="flex items-center gap-4">
          <span
            class="text-[12px] font-black text-primary bg-primary/5 px-5 py-2 rounded-full border border-primary/10 tracking-widest uppercase"
          >
            {{ service.categoryLabel }}
          </span>
          <div
            class="flex-grow h-[1px] bg-gradient-to-r from-primary/20 to-transparent"
          />
        </div>
        <h3
          class="text-3xl lg:text-4xl font-black text-gray-900 leading-[1.1] tracking-tight"
        >
          {{ service.name }}
        </h3>
        <p
          class="text-gray-500 text-base md:text-xl font-medium leading-relaxed max-w-xl"
        >
          {{ service.description }}
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div
          ...
          :key="idx"
          class="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl border border-transparent hover:border-primary/20 hover:bg-white hover:shadow-xl transition-all duration-500 group/feat"
        >
          <Icon
            name="fa6-solid:circle-check"
            class="text-primary text-base flex-shrink-0 group-hover/feat:scale-125 transition-transform"
          />
          <span
            class="text-[14px] font-bold text-gray-700 uppercase tracking-tight"
            >{{ feature }}</span
          >
        </div>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-8 pt-4">
        <div class="group/price cursor-default">
          <p
            class="text-[11px] font-black text-gray-400 mb-2 uppercase tracking-widest"
          >
            Giá dịch vụ
          </p>
          <p
            class="text-4xl font-black text-primary transition-transform group-hover/price:scale-105 origin-left"
          >
        ...
        <div class="group/time cursor-default">
          ...
          <p
            class="text-2xl font-black text-gray-900 transition-transform group-hover/time:scale-105 origin-left"
          >
      ...
      <div class="pt-6">
        ...
        >
          ...
          <Icon
            ...
            class="relative z-10 group-hover:translate-x-3 transition-transform text-base"
          />
          <div
