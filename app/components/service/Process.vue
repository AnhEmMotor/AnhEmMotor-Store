<template>
	<section ref="processRef" class="bg-gray-50 py-32 border-y border-gray-100 overflow-hidden relative">
		<!-- Background Accents -->
		...
		<div class="absolute bottom-0 left-0 w-[300px] h-[300px] bg-primary/5 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/2"></div>
		...
		<div class="max-w-7xl mx-auto px-6 sm:px-10 lg:px-12 relative z-10">
			<div class="grid grid-cols-1 lg:grid-cols-12 gap-20 items-center">
				...
				>
					<div class="relative z-10 rounded-[4rem] overflow-hidden shadow-2xl border-[12px] border-white group">
						<img src="/service-process.png" alt="Customer Service" class="w-full h-auto object-cover transform group-hover:scale-110 transition-transform duration-1000" />
						<div class="absolute inset-0 bg-gradient-to-t from-primary/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
					</div>
					<!-- Experience Badge -->
				...
				>
					<div class="space-y-6">
						<h2 class="text-5xl md:text-6xl font-[1000] text-gray-900 leading-[0.9] tracking-tight">
					...
					<div class="space-y-6">
						<div v-for="(step, idx) in steps" :key="idx" 
							class="flex gap-8 p-8 bg-white border border-gray-100 rounded-[3rem] hover:border-primary/30 hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 group"
							:style="{ transitionDelay: `${idx * 200 + 500}ms` }"
							:class="isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'"
						>
							<div class="flex-shrink-0 w-20 h-20 bg-primary/5 rounded-[1.5rem] flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-all duration-500 transform group-hover:rotate-6 shadow-sm">
								<Icon :name="step.icon" class="text-4xl text-primary group-hover:text-white" />
							</div>
							<div class="space-y-3">
								<div class="flex items-center gap-4">
									<span class="text-[10px] font-black text-primary uppercase tracking-[0.2em]">Bước 0{{ idx + 1 }}</span>
									<div class="w-12 h-[1px] bg-primary/20"></div>
								</div>
								<h3 class="text-2xl font-black text-gray-900 tracking-tight">{{ step.title }}</h3>

								<p class="text-gray-500 text-sm font-medium leading-relaxed max-w-lg">{{ step.desc }}</p>
							</div>
						</div>
					</div>
				</div>
