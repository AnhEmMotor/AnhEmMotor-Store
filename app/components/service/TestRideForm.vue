<template>
	<div class="test-ride-form-container">
		...
		<form v-if="!isSuccess" class="space-y-6" @submit.prevent="handleSubmit">
			...
			<div class="space-y-2">
				...
				<div class="relative">
					...
					<Icon
						...
						class="absolute right-5 top-1/2 -translate-y-1/2 text-gray-400 text-[10px] pointer-events-none"
					/>
				</div>
			</div>
				...
				<div class="space-y-2">
					...
					<div class="relative">
						<select
							...
							class="w-full px-4 py-4 bg-gray-50 border-none rounded-2xl text-[10px] font-black focus:ring-2 focus:ring-red-500/20 transition-all appearance-none uppercase tracking-wider"
						>
						...
						<Icon
							name="fa6-solid:chevron-down"
							class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 text-[10px] pointer-events-none"
						/>
					</div>
				</div>
				<div class="space-y-2">
			...
			<div class="pt-2">
				<label
					class="flex items-center gap-4 p-5 bg-red-50 rounded-2xl border border-red-100 cursor-pointer group hover:bg-red-100/50 transition-all"
				>
					...
					<span
						class="text-[10px] font-black text-gray-900 uppercase tracking-wider select-none"
					>
			...
			>
				...
				<Icon
					...
					class="text-[10px] group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform"
				/>
			</button>
		</form>
		...
		<div v-else class="text-center py-10 space-y-6">
			<div
				class="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg"
			>
			...
			<p
				class="text-gray-500 text-xs font-medium leading-relaxed max-w-xs mx-auto"
			>
				Chúng tôi đã nhận được yêu cầu và sẽ liên hệ xác nhận trong vòng 30
				phút.
			</p>
			<button
				class="text-xs font-black text-red-600 uppercase tracking-widest hover:underline"
				@click="$emit('close')"
			>
				Đóng
			</button>
