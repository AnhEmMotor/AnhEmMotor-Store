<template>
	...
	>
		<div
			class="absolute left-0 top-10 bottom-10 w-0 group-hover:w-[4px] bg-red-600 transition-all duration-300 rounded-r-full"
		/>

		<div class="flex justify-between items-start mb-6">
			<div
				class="p-3 bg-red-600/5 rounded-xl group-hover:scale-110 transition-transform flex items-center justify-center w-12 h-12 shadow-sm"
			>
				<Icon
			...
			<div class="flex flex-col items-end gap-1">
				<span
					class="text-[9px] font-black text-yellow-600 bg-yellow-50 border border-yellow-200 px-3 py-1 rounded-lg"
				>
					{{ service.estimatedTime || "30-60" }} Phút
				</span>
				<span
					v-if="service.isFeatured"
					class="text-[8px] font-black text-white bg-red-600 px-2 py-0.5 rounded-md"
				>
					Phổ biến
				</span>
			</div>
		...
		<div class="space-y-4 flex-grow">
			<div class="space-y-2">
				...
				<h3
					class="text-lg font-black text-gray-900 group-hover:text-red-600 transition-colors leading-tight"
				>
					{{ service.name }}
				</h3>
			</div>
			...
			<p
				class="text-gray-500 text-xs font-medium leading-relaxed line-clamp-2 min-h-[32px]"
			>
				{{ service.description }}
			</p>
			...
			<ul class="space-y-3 pt-4">
				<li
					...
					:key="idx"
					class="flex items-start gap-3 text-[10px] font-bold text-gray-500 leading-tight"
				>
					<Icon
