<template>
	<div
		class="sticky top-[64px] lg:top-[72px] z-30 bg-white/80 backdrop-blur-xl border-b border-gray-100/50 shadow-[0_4px_20px_-5px_rgba(0,0,0,0.05)] w-full py-2.5 font-['Manrope']"
	>
		<div
			ref="scrollContainer"
		...
		>
			<button
				...
				:class="[ activeCategory === cat.id ?'bg-primary text-white shadow-lg shadow-primary/30 border-primary scale-105'
			...
			>
				...
				<div
					...
					class="absolute -top-1 -right-1 w-2 h-2 bg-white rounded-full shadow-[0_0_8px_#fff] animate-pulse"
				/>
			</button>
		</div>
		...
		<div class="hidden lg:flex justify-center">
			<div
				class="flex flex-wrap items-center justify-center gap-2.5 p-1.5 bg-gray-50/50 backdrop-blur-xl rounded-2xl border border-gray-100 shadow-sm max-w-5xl"
			>
				<button
					v-for="cat in categories"
					:key="cat.id"
					...
					:class="[ activeCategory === cat.id ?'bg-primary text-white shadow-lg scale-105 z-20 shadow-primary/20'
