...
export function useMortgage() {
  ...
  const downPaymentValue = computed(() => (bikePrice.value * downPaymentPercent.value) / 100);
  const loanAmount = computed(() => bikePrice.value - downPaymentValue.value);

  const rawMonthlyPayment = computed(() => {
    const totalInterest = loanAmount.value * interestRate * loanTermMonths.value;
    return Math.round((loanAmount.value + totalInterest) / loanTermMonths.value);
  });
