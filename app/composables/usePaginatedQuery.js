...
export function usePaginatedQuery(config) {
  ...
  const buildCleanFilters = () => {
    ...
    for (const key of filterKeys) {
      const val = debouncedFields.includes(key) ? debouncedValues[key] : filtersObj[key];
  ...
  if (import.meta.server) {
    onServerPrefetch(async () => {
      ...
      const fetchKey = [...queryKey, { ...queryParams.value, page: currentPage }];
        ...
        if (currentPage < total) {
          neighborPromises.push(
            queryClient.prefetchQuery({
              queryKey: [...queryKey, { ...queryParams.value, page: currentPage + 1 }],
  ...
  const { data, isLoading, isFetching, isPlaceholderData, status, ...restQuery } = useQuery({
  ...
  const listData = computed(() => {
    ...
    return typeof dataKey === 'function' ? dataKey(data.value) : data.value[dataKey] || [];
  });
