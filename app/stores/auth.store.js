...
export const useAuthStore = defineStore('auth', () => {
          ...
          if (data) {
            ...
            if (data.userName) {
              ...
              if (import.meta.client) {
                ...
                if (currentRoute.meta.permission && !hasPermission(currentRoute.meta.permission)) {
                  return navigateTo('/');
                }
  ...
  async function _performRefresh() {
    try {
      ...
      const response = await $fetch.raw(`${backendUrl}/api/v1/auth/refresh-token`, {
        method: 'POST',
        headers: headers,
        credentials: 'include',
      });
      ...
      let newCookieString = '';
      if (import.meta.server && mainRequestEvent && response.headers.has('set-cookie')) {
      ...
      if (data && data.accessToken) {
        ...
        if (import.meta.server && mainRequestEvent?.context) {
          mainRequestEvent.context._freshRefreshToken = data.refreshToken || null;
        }
