...
export const useHomeStore = defineStore('home', () => {
  ...
  const filteredProducts = computed(() => {
    return featuredProducts.value.filter((p) => {
      ...
      const matchesKeyword =
        ...
        p.name.toLowerCase().includes(searchFilters.value.keyword.toLowerCase());
      const matchesPrice =
        !searchFilters.value.priceRange || checkPriceRange(p.price, searchFilters.value.priceRange);
  ...
  const fetchHomeData = async () => {
    ...
    try {
      const [rawBrands, rawStats, rawPhrases, rawProducts, rawBanners] = await Promise.all([
