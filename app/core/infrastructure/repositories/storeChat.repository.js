export const storeChatRepository = {
  ...
  async getHistory(sessionId) {
    const axios = useAxios();
    const response = await axios.get(`/api/v1/store-chat/sessions/${sessionId}/history`);
    return response.data;
  },
  ...
  async setContactInfo(sessionId, contactName, contactPhone) {
    const axios = useAxios();
    const response = await axios.post(`/api/v1/store-chat/sessions/${sessionId}/contact-info`, {
      contactName,
      contactPhone,
    });
    return response.data;
