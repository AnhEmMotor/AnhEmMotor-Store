...
test("support requests derive a real operational category from the Store subject", () => {
  assert.equal(
    resolveContactCategory({ subject: "Dịch vụ bảo dưỡng" }),
    "Service",
  );
  assert.equal(
    resolveContactCategory({ subject: "Phản ánh dịch vụ" }),
    "AfterSales",
  );
  assert.equal(resolveContactCategory({ subject: "Tư vấn mua xe" }), "Sales");
  assert.equal(
    resolveContactCategory({ subject: "Tư vấn mua xe", category: "Other" }),
    "Other",
  );
});
